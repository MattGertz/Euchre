...
namespace MAUIEuchre
{
    public class EuchrePlayer
    {
        ...
        private int LowestCardThatTakesLedSuit(IGameTable Table, EuchreCard.Values ValueToBeat)
        {
            int index = -1;
        ...
        private int LowestCardTrumpThatTakes(IGameTable Table, EuchreCard.Values ValueToBeat)
        {
            int index = -1;
            int DifferenceToMinimize = (int)EuchreCard.Values.RightBower + 1;
            for (int i = 0; i <= 4; i++)
        ...
        public EuchrePlayer(Seats NewSeat, IGameTable table)
        {
            Seat = NewSeat;
            _gameTable = table;
        ...
        public string GetDisplayName()
        {
            ...
            default: throw new Exception("Unknown seat type in GetDisplayName");
            }
        }

        public void HumanBidFirstRound()
        {
            bool forceGoAlone = _gameTable.handDealer == Seats.Partner && _gameTable.ruleUseQuietDealer;
            _gameTable.ShowBidFirstRound(forceGoAlone);
        }
        ...
        {
            bool forceGoAlone = _gameTable.handDealer == Seats.Partner && _gameTable.ruleUseQuietDealer;
            _gameTable.ShowBidSecondRound(
                _gameTable.handKitty[0].Suit,
                _gameTable.handDealer,
                Seat == _gameTable.handDealer && _gameTable.ruleStickTheDealer,
                forceGoAlone);
        }
